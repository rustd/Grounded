...
const PLUGIN_PAGES: Record<string, PluginPage> = {
  ...
  lakebase: {
    navLabel: 'Decision Loop',
    path: '/',
    expectedTexts: ['Maternal outreach decision cockpit', 'Agent recommendation', 'Based on these records', 'Commit action record'],
  },
  genie: {
    navLabel: 'Genie',
    path: '/genie',
...
test('smoke test - app loads and displays home page', async ({ page }) => {
  ...
  await expect(
    page.getByRole('heading', { name: 'HackEvent', exact: true }),
  ).toBeVisible();
  await expect(page.getByRole('heading', { name: 'Maternal outreach decision cockpit' })).toBeVisible();

  await expect(page.getByRole('link', { name: 'Decision Loop' })).toBeVisible();
